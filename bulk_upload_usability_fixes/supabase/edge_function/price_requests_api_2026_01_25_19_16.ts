...
serve(async (req) => {
  ...
  try {
    ...
    const action = pathSegments[0] || url.searchParams.get('action');
    const id = pathSegments[1] || url.searchParams.get('id');
    const projectId = url.searchParams.get('project_id');

    console.log('Price requests API called:', { method, action, id, projectId });
